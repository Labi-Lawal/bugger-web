...
export default validateToken(function deleteTask(req:Request, res:NextApiResponse) {
    if(req.method !== 'POST') return res.status(405).json({ message: 'Only DELETE request allowed' });
    ...
    ProjectModel.findOneAndUpdate(
        ...
        return res.status(400).json({ message: "Task deleted successfully", project: updatedProject });
    })
    .catch((error)=> {
