...
const validateToken = (handler:any)=> {
    return async (req:Request, res:any)=> {
        ...
        if(!authToken) return res.status(403).send({status: 403, message: 'No token provided'});
        if(authToken.split(' ')[0] !== 'Bearer') return res.status(403).send({status: 403, message: 'Invalid token'});
